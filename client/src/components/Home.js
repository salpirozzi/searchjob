            ...
            <React.Fragment>
                    ...
                    <div className="nav-user">
                        ...
                        ?
                            <React.Fragment>
                                ...
                                <Link to='/register' className="nav-user-link">
                                    Registrati
                                </Link>
                            </React.Fragment>       
                        :
                            <React.Fragment>
                                <Link to='/login' className="nav-user-link">
                                    <strong>{this.props.user_details.username}</strong>
                                </Link>
                                <a 
                                    href="/#" 
                                    onClick={this.logOut} 
                                ...
                                <Link to='/insert' className="nav-user-link success">
                                    <i className="fa fa-plus" /> Inserisci un annuncio
                                </Link>
                            </React.Fragment>
                        }
                    </div> 
                ...
                <section className="filters">
                    <select 
                        name="date" 
                        id="date" 
                        value={this.state.date !== null ? this.state.date : "null"} 
                        onChange={(e) => this.setFilter(e, this.state.contract, this.state.location, e.target.value)}
                    >
                        <option value="null">
                    ...
                    <select 
                        name="location" 
                        id="location" 
                        value={this.state.location !== null ? this.state.location : "null"} 
                        onChange={(e) => this.setFilter(e, this.state.contract, e.target.value, this.state.date)}
                    >
                        <option value="null">
                            Località
                        </option>
                        {this.state.showed_adverts.length > 0 && locations.map(
                            (i) =>  (
                    ...
                    <select 
                        name="contract" 
                        id="contract" 
                        value={this.state.contract !== null ? this.state.contract : "null"} 
                        onChange={(e) => this.setFilter(e, e.target.value, this.state.location, this.state.date)}
                    >
                        <option value="null">
                            Tipologia contratto
                        </option>
                        {this.state.showed_adverts.length > 0 && contracts.map(
                            (i) =>  (
                                        <option key={i} value={i}>
                                            {i}
                                        </option>
                ...
                {this.state.showed_adverts.length < 1 && 
                    <img src="https://i.gifer.com/ZZ5H.gif" alt="Loading" className="loading-img"/>
                }

                {this.state.page_adverts.length < 1 && this.state.showed_adverts.length > 0 && 
                    <div className="advert-list">Nessun annuncio trovato.</div>
                }

                {this.state.page_adverts.length > 0 && 
                    <section className="advert-list">
                        {this.state.page_adverts.map(
                            (announce, idx) =>
                                <div className="advert" key={idx}>
                                    ...
                                    <div className="advert-data">
                                        <a 
                                            href="/#" 
                                            onClick={(e) => this.showAdvert(e, announce)} 
                                            id={idx} 
                                            key={idx}
                                        >
                                            {announce.title}
                                        ...
                                        <div className="advert-published">
                                            ...
                                            {announce.month_salary > 0 && 
                                                <p>
                                                    {announce.month_salary} € / mese
                                                </p>
                                            }
                                        </div>
                                    </div>
                ...
                {this.state.advert_info !== null && 
                    <section className="menu">
                        <a 
                            ...
                            onClick={(e) => this.showAdvert(e, null)}
                        >
                        ...
                        <Link to={{
                            ...
                            <h3>
                                {this.state.advert_info.title} <i className="fa fa-arrow-right" />
                            </h3>
                        </Link>
                        <Item 
                            advert_info={this.state.advert_info} 
                ...
                {this.state.total_pages > 1 && 
                    <section className="pagination">
                        <button 
                            type="button" 
                            onClick={(e) => this.goPage(e, this.state.current_page - 1)}
                        >
